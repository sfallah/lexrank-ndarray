...
pub fn wide_normalize_l2(c: &mut Criterion, dataset: &str, tensor_file: &str) {
    ...
    c.bench_function(format!("wide_normalize_l2 {}", dataset).as_str(), |b| {
        b.iter(|| {
            embeds_vec.par_iter().for_each(|(shape, vec)| {
                let mut embeddings = flatten_vec_to_wide_matrix(vec, shape[0], shape[1]).unwrap();
                normalize_l2_wide_new(&mut embeddings);
            });
        });
    });
}
...
pub fn benches() {
    let num_threads = num_cpus::get(); // Replace with the number of threads you want
